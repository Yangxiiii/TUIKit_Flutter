...
class AISubtitle extends StatefulWidget {
  ...
  const AISubtitle({super.key, required this.userId});
  ...
  late var trtcCloudListener = TRTCCloudListener(
    onRecvCustomCmdMsg: (userId, cmdId, seq, message) {
      ...
      try {
        ...
        Log.getCommonLog(
          "CallAISubtitle",
        ).error("Parse custom message failed: ${e.toString()}");
      }
    },
  );

  void _updateTranslationInfo(
    String roundId,
    String sender,
    String text,
    ...
    final index = _translationInfos.indexWhere(
      (info) => info.roundId == roundId,
    );

    if (index != -1) {
      final existingInfo = _translationInfos[index];
      existingInfo.sender = sender.contains(AI_TRANSLATION_ROBOT)
          ? existingInfo.sender
          : sender;
                        ...
                        builder: (context, snapshot) {
                          ...
                          return Padding(
                            padding: EdgeInsets.only(
                              bottom: index == _translationInfos.length - 1
                                  ? 0
                                  : 8,
                            ),
                            child: RichText(
